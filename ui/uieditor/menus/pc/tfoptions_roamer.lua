...
DataSources.TFOptionsRoamerP1 = DataSourceHelpers.ListSetup("PC.TFOptionsRoamerP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Roamer Enabled", description = "Roamer allows you to wait at the end of every round, before the next round starts. No more crawlers! To start the next round, press ADS + Melee", profileVarName = "roamer_enabled", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Roamer Time Between Rounds", description = "Choose how long you have between rounds.", profileVarName = "roamer_time", datasource = "TFOptionsRoamerTime", widgetType = "dropdown"}, properties = CoD.TFPCUtil.OptionsGenericDropdownProperties})
	return f26_local0
end, true)
...
DataSources.TFOptionRoamerCategories = DataSourceHelpers.ListSetup("PC.TFOptionRoamerCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "ROAMER OPTIONS", tabWidget = "CoD.TFOptions_Roamer_P1"}})
...
LUI.createMenu.TFOptionsRoamerPage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsRoamerPage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsRoamerPage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionRoamerCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsRoamerPage.buttonPrompts"))
	end)
	if f0_local3 then
