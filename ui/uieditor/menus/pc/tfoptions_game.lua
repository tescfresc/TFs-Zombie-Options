...
DataSources.TFOptionsGameP1 = DataSourceHelpers.ListSetup("PC.TFOptionsGameP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Starting Round", description = "Choose which round to start on.", profileVarName = "starting_round", lowValue = 1, highValue = 255, useIntegerDisplay = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Zombie Counter Enabled", description = "Adds a zombie counter at the top left of the screen to show you how many zombies are left in the wave.", profileVarName = "zcounter_enabled", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "No Round Delay", description = "Removes the delay between rounds. Obviously pointless if you have roamer enabled.", profileVarName = "no_round_delay", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Zombie Hitmarkers", description = "Show hitmarkers when damaging zombies.", profileVarName = "hitmarkers", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Timed Gameplay", description = "Enables timed gameplay mode. When active, zombies don't stop spawning and the goal is to last as long as possible. Not compatible with roamer or the zombie counter.", profileVarName = "timed_gameplay", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Start with All Doors Open", description = "Opens all doors on the map.", profileVarName = "open_all_doors", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Mystery Box in All Locations", description = "Spawns the mystery box in every possible location at once. Prevents teddies.", profileVarName = "every_box", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Start with Power Enabled", description = "Turn on the power when the game starts.", profileVarName = "start_power", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	return f26_local0
end, true)

--[[ DataSources.TFOptionsGameP2 = DataSourceHelpers.ListSetup("PC.TFOptionsGameP2", function (table_list)
	local table_list = {}
	table.insert(table_list, {models = {label = "Startingddd Round", description = "Choose which round to start on.", profileVarName = "starting_round", lowValue = 1, highValue = 255, useIntegerDisplay = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(table_list, {models = {label = "Zombie Cdddounter Enabled", description = "Adds a zombie counter at the top left of the screen to show you how many zombies are left in the wave.", profileVarName = "zcounter_enabled", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(table_list, {models = {label = "No Round dddDelay", description = "Removes the delay between rounds. Obviously pointless if you have roamer enabled.", profileVarName = "no_round_delay", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(table_list, {models = {label = "Zombie Hidddtmarkers", description = "Show hitmarkers when damaging zombies.", profileVarName = "hitmarkers", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(table_list, {models = {label = "Timed ddd", description = "Enables timed gameplay mode. When active, zombies don't stop spawning and the goal is to last as long as possible. Not compatible with roamer or the zombie counter.", profileVarName = "timed_gameplay", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	return table_list
end, true) ]]
...
--[[ DataSources.TFOptionsGameP2.getWidgetTypeForItem = function (f27_arg0, f27_arg1, f27_arg2)
	if f27_arg1 then
		local f27_local0 = Engine.GetModelValue(Engine.GetModel(f27_arg1, "widgetType"))
		if f27_local0 == "dropdown" then
			return CoD.OptionDropdown
...
DataSources.TFOptionGameCategories = DataSourceHelpers.ListSetup("PC.TFOptionGameCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "GAME OPTIONS", tabWidget = "CoD.TFOptions_Game_P1"}})
	--table.insert(f28_local0, {models = {tabName = "PAGE 2", tabWidget = "CoD.TFOptions_Game_P2"}})
...
LUI.createMenu.TFOptionsGamePage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsGamePage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsGamePage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionGameCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsGamePage.buttonPrompts"))
	end)
	if f0_local3 then
