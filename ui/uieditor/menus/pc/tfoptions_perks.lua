...
DataSources.TFOptionsPerksP1 = DataSourceHelpers.ListSetup("PC.TFOptionsPerksP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Remove Perk Limit", description = "Let the player buy unlimited perks.", profileVarName = "no_perk_lim", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Bigger Mulekick", description = "Mulekick now lets you carry 4 weapons.", profileVarName = "bigger_mule", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Spawn With Perkaholic", description = "Choose whether to start with perkaholic.", profileVarName = "perkaholic", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Spawn with Quick Revive", description = "Choose whether to start with quick revive.", profileVarName = "spawn_with_quick_res", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
		return f26_local0
end, true)
...
DataSources.TFOptionPerksCategories = DataSourceHelpers.ListSetup("PC.TFOptionPerksCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "PERK OPTIONS", tabWidget = "CoD.TFOptions_Perks_P1"}})
...
LUI.createMenu.TFOptionsPerksPage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsPerksPage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsPerksPage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionPerksCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsPerksPage.buttonPrompts"))
	end)
	if f0_local3 then
