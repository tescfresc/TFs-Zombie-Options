...
DataSources.TFOptionsZombiesP1 = DataSourceHelpers.ListSetup("PC.TFOptionsZombiesP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Extra Points Per Kill", description = "Get extra points for killing zombies.", profileVarName = "extra_cash", lowValue = 0, highValue = 100, useMultipleOf10Display = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Extra Points Per Melee Kill", description = "Get extra points for killing zombies by melee.", profileVarName = "melee_bonus", lowValue = 0, highValue = 100, useMultipleOf10Display = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Extra Points Per Headshot Kill", description = "Get extra points for killing zombies by headshots.", profileVarName = "headshot_bonus", lowValue = 0, highValue = 100, useMultipleOf10Display = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Max Spawned Zombies", description = "Maximum number of zombies that can be spawned at any time. 24 is the default. Higher numbers can cause issues with lag. Highest value I've tested is 40.", profileVarName = "max_zombies", lowValue = 1, highValue = 64, useIntegerDisplay = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Weaker Zombies", description = "Zombie's strength increases slower.", profileVarName = "weaker_zombs", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "Zombies Always Sprint", description = "Zombies will always sprint, even on round 1.", profileVarName = "zombs_always_sprint", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	table.insert(f26_local0, {models = {label = "No Zombie Spawn Delay", description = "Sets the zombie spawn delay to 0. With this, zombies spawn instantly.", profileVarName = "no_delay", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	return f26_local0
end, true)
...
DataSources.TFOptionZombiesCategories = DataSourceHelpers.ListSetup("PC.TFOptionZombiesCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "ZOMBIE OPTIONS", tabWidget = "CoD.TFOptions_Zombies_P1"}})
...
LUI.createMenu.TFOptionsZombiesPage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsZombiesPage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsZombiesPage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionZombiesCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsZombiesPage.buttonPrompts"))
	end)
	if f0_local3 then
