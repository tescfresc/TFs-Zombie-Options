...
DataSources.TFOptionsPlayerP1 = DataSourceHelpers.ListSetup("PC.TFOptionsPlayerP1", function (f26_arg0)
	local f26_local0 = {}
	table.insert(f26_local0, {models = {label = "Starting Points", description = "Choose how many points to start a game with.", profileVarName = "starting_points", datasource = "TFOptionsStartingPoints", widgetType = "dropdown"}, properties = CoD.TFPCUtil.OptionsGenericDropdownProperties})
	table.insert(f26_local0, {models = {label = "Move Speed Multiplier", description = "Tweak the player speed. 1 is default.", profileVarName = "move_speed", lowValue = 0.1, highValue = 2, isFloat = 1, widgetType = "slider"}, properties = CoD.TFPCUtil.OptionsGenericSliderProperties})
	table.insert(f26_local0, {models = {label = "Player Health", description = "Choose how many hits from a zombie it takes to down the player.", profileVarName = "higher_health", datasource = "TFOptionsPlayerHealth", widgetType = "dropdown"}, properties = CoD.TFPCUtil.OptionsGenericDropdownProperties})
	table.insert(f26_local0, {models = {label = "EXO Movement", description = "Enable EXO movement. (Probably won't work on any maps that use EXO movement, such as SoE or Der Eisendrache)", profileVarName = "exo_movement", widgetType = "tfcheckbox"}, properties = CoD.TFPCUtil.OptionsGenericCheckboxProperties})
	return f26_local0
end, true)
...
DataSources.TFOptionPlayerCategories = DataSourceHelpers.ListSetup("PC.TFOptionPlayerCategories", function (f28_arg0)
	local f28_local0 = {}
	table.insert(f28_local0, {models = {tabIcon = CoD.buttonStrings.shoulderl}, properties = {m_mouseDisabled = true}})
	table.insert(f28_local0, {models = {tabName = "PLAYER OPTIONS", tabWidget = "CoD.TFOptions_Player_P1"}})
...
LUI.createMenu.TFOptionsPlayerPage = function (InstanceRef)
	local HudRef = CoD.Menu.NewForUIEditor("TFOptionsPlayerPage")
	if f0_local2 then
	...
	HudRef.buttonModel = Engine.CreateModel(Engine.GetModelForController(InstanceRef), "TFOptionsPlayerPage.buttonPrompts")
	...
	f31_local7.grid:setDataSource("TFOptionPlayerCategories")
	...
	LUI.OverrideFunction_CallOriginalSecond(HudRef, "close", function (Sender)
		...
		Engine.UnsubscribeAndFreeModel(Engine.GetModel(Engine.GetModelForController(InstanceRef), "TFOptionsPlayerPage.buttonPrompts"))
	end)
	if f0_local3 then
